...
class CheckpointManager(AbstractCheckpointManager, epy.ContextManager):
  ...
  def __init__(
    ...
    self._save_progess_tracker = synchronization.ThreadSaveMultiHostValueHolder(
        value=False,
  ...
  def save(
    ...
    self._save_progess_tracker.set_value(True)
  ...
  def is_saving_in_progress(self) -> bool:
    ...
    return self._save_progess_tracker.get_value()
  ...
  def _finalize(self, step: int, steps_to_remove: List[int]):
    ...
    try:
      ...
      self._save_progess_tracker.set_value(False)
      logging.info(
          '[process=%s][thread=%s][step=%s] CheckpointManager Save Finalize is'
          ' done on all hosts.',
          process_index,

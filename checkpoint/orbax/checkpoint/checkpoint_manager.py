...
class CheckpointManager(AbstractCheckpointManager, epy.ContextManager):
  ...
  def __init__(
    ...
    Example::
      ...
      # Single, unnamed (default) item, no need to specify `item_names`.
      with CheckpointManager(
          'path/to/dir/',
    ...
    Args:
      ...
      checkpointers: a mapping of object name to Checkpointer object. For
        ...
        with a single, unnamed (default) item rather than a dictionary of items.
        See below for more details. `item_names` and `checkpointers` are
        mutually exclusive - do not use together. Also, please don't use
        `checkpointers` and `item_handlers` together.
      ...
      item_handlers: A mapping of item name to `CheckpointHandler`. The mapped
        ...
        `save` and `restore` should always be called in a default item context.

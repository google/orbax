...
class InternalTreeMetadataEntryTest(parameterized.TestCase):
  ...
  def test_invalid_user_metadata(self, user_metadata):
    tree = {'scalar_param': 1}
    with self.assertRaisesRegex(TypeError, 'Failed to encode'):
      tree_metadata_lib.InternalTreeMetadata.build(
          param_infos=_to_param_infos(tree), user_metadata=user_metadata
      )
  ...
  def test_user_metadata(self, user_metadata):
    tree = {'scalar_param': 1}
    internal_tree_metadata = tree_metadata_lib.InternalTreeMetadata.build(
        param_infos=_to_param_infos(tree), user_metadata=user_metadata
    )
    self.assertEqual(internal_tree_metadata.user_metadata, user_metadata)
...
class TreeMetadataTest(parameterized.TestCase):
  ...
  def test_properties(self, tree):
    ...
    metadata = tree_metadata_lib._TreeMetadataImpl(
        tree=tree, user_metadata=user_metadata
    )
    self.assertDictEqual(metadata.user_metadata, user_metadata)

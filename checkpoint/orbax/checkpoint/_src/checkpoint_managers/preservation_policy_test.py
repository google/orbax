...
class PreservationPolicyTest(parameterized.TestCase):
  ...
  @parameterized.parameters(
      dict(
          n=None,
          all_metrics={'loss': [5, None, 3, None, 7]},
          expected_preserved_steps=[0, 1, 2, 3, 4],
      ),
      dict(
          n=1,
          all_metrics={'loss': [7, None, 4, None, 5]},
          expected_preserved_steps=[1, 2, 3],
      ),
      dict(
          n=4,
          all_metrics={'loss': [5, None, 4, None, 7]},
          expected_preserved_steps=[0, 1, 2, 3, 4],
      ),
  )
  def test_best_n_policy(self, n, all_metrics, expected_preserved_steps):
    ...
    for i, checkpoint in enumerate(checkpoints):
      if all_metrics['loss'][i]:
        checkpoint.metrics = {'loss': all_metrics['loss'][i]}

...
class _StandardNameFormat(NameFormat[Metadata]):
  ...
  def _glob_step_paths(self, base_path: epath.PathLike) -> list[epath.Path]:
    ...
    if self.enable_hns and gcs_utils.is_hierarchical_namespace_enabled(
      ...
      logging.info(
          'Found blob prefixes for bucket %s: %s', bucket.name, result.prefixes
      )
      step_paths = [
          epath.Path(base_path) / folder
          for folder in result.prefixes
          if folder.startswith(self.step_prefix)
      ]
      logging.info('Found step paths: %s', step_paths)
      return step_paths
    else:
      return list(

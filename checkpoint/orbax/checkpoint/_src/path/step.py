...
is_checkpoint_finalized = lambda *a, **k: asyncio_utils.run_sync(
    temporary_paths.is_path_finalized(*a, **k)
)
is_tmp_checkpoint = lambda *a, **k: asyncio_utils.run_sync(
    temporary_paths.is_path_temporary(*a, **k)
)
tmp_checkpoints = lambda *a, **k: asyncio_utils.run_sync(
...
def find_step_path(
  ...
  if is_gcs_path(base_path):
    ...
    for uncommitted_name in temporary_paths.all_temporary_paths(base_path):
...
class _StandardNameFormat(NameFormat[Metadata]):
  ...
  def _build_metadata(
    ...
    if not temporary_paths.is_path_finalized(step_path):
      return None
...
def _is_legacy_finalized_step_checkpoint(path: epath.Path) -> bool:
  return _is_legacy_step_checkpoint(
      path
  ) and temporary_paths.is_path_finalized(path)

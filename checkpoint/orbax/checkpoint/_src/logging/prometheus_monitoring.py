...
"""Prometheus metrics exporter for Orbax.

This module enables Orbax to export performance and operational metrics to
Prometheus, allowing users to monitor checkpointing behavior in open-source
environments.

To enable telemetry collection, set the environment variable
`ENABLE_ORBAX_PROMETHEUS_TELEMETRY=true`. By default, telemetry collection
is disabled.
"""
...
enable_telemetry = os.environ.get('ENABLE_ORBAX_PROMETHEUS_TELEMETRY', 'false')
...
class PrometheusMonitoring(monitoring.MetricRecorder):
  ...
  def record_event(self, metric_name: str, **kwargs: Any) -> None:
    ...
    metric = self._get_or_create_metric(
        metric_name, metric_name_safe, _prom_counter, labelnames
    )
    if _prom_counter and isinstance(metric, _prom_counter):
      if labelnames:
        try:
          ...
          logging.warning(
              'Failed to record Prometheus event "%s" due to label mismatch:'
              ' %s. Provided keys: %s',
              metric_name,
              e,
              sorted_keys,
  ...
  def record_scalar(
    ...
    metric = self._get_or_create_metric(
        metric_name, metric_name_safe, _prom_histogram, labelnames
    )
    if _prom_histogram and isinstance(metric, _prom_histogram):
      if labelnames:
        try:
          ...
          logging.warning(
              'Failed to record Prometheus scalar "%s" due to label mismatch:'
              ' %s. Provided keys: %s',
              metric_name,
              e,
              sorted_keys,
    ...
    if _prom_histogram and isinstance(metric, _prom_histogram):
      if labelnames:
        try:
          ...
          logging.warning(
              'Failed to record Prometheus duration "%s" due to label mismatch:'
              ' %s. Provided keys: %s',
              metric_name,
              e,
              sorted_keys,
  ...
  def _is_allowed(self, metric_name: str) -> bool:
    ...
    return metric_name.startswith(self._allowed_prefixes)

  def _get_or_create_metric(
      self,
      metric_name: str,
      metric_name_safe: str,
      metric_class: Any,
      labelnames: tuple[str, ...],
  ) -> Any:
    """Gets an existing metric or creates a new one thread-safely."""
    if metric_name_safe not in self._metrics:
      with self._lock:
        if metric_name_safe not in self._metrics:
          try:
            self._metrics[metric_name_safe] = metric_class(
    ...
    return self._metrics[metric_name_safe]

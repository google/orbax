...
def _construct_restore_args(
    target: PyTree | None,
    metadata_tree: tree_metadata.TreeMetadata,
  ...
  Args:
    ...
    metadata_tree: The metadata tree to be used as target if target is none or
      as fallback sharding.
  ...
  """
  flat_metadata = tree_utils.to_flat_dict(metadata_tree.tree)
  ...
  def _get_sharding_for_target_leaf(
    ...
    if isinstance(item_leaf, (jax.ShapeDtypeStruct, jax.Array)):
      if support_layout:
        return arrays_sharding_lib.get_sharding_or_format(item_leaf)
      else:
        if hasattr(item_leaf, 'sharding') and item_leaf.sharding is not None:
          return item_leaf.sharding

    # 2. If item_leaf.sharding is None, try to get from metadata_tree
    metadata_leaf = flat_metadata.get(target_path_tuple)
  ...
  if target is not None:
    ...
    target_tree = metadata_tree.tree
    sharding_tree = jax.tree_util.tree_map(
        _get_sharding_from_metadata_leaf, metadata_tree.tree
    )

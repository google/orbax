...
class BasePyTreeCheckpointHandler(
  ...
  def __init__(
    ...
    self._metadata_manager = metadata_manager or metadata_manager_lib.MetadataManager(
  ...
  def restore(
    ...
    internal_tree_metadata = asyncio_utils.run_sync(
        self._metadata_manager.read_metadata_file(directory)
    )
  ...
  async def _write_metadata_after_commits(
    ...
    tasks = [
        self._metadata_manager.write_metadata_file(
            checkpoint_dir,
  ...
  def finalize(self, directory: epath.Path) -> None:
    ...
    """
    asyncio_utils.run_sync(self._metadata_manager.finalize_async(directory))

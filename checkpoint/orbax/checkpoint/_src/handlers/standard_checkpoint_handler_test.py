...
StandardCheckpointHandler = (
    standard_checkpoint_handler_test_utils.StandardCheckpointHandler
)
...
class StandardCheckpointHandlerTestBase(
  ...
  def setUp(self):
    ...
    test_utils.sync_global_processes('StandardCheckpointHandler:setup_complete')

  def tearDown(self):
    test_utils.sync_global_processes('StandardCheckpointHandler:tests_complete')
    self.handler.close()
    super().tearDown()
  ...
  def test_topology_mismatch(self):
    ...
    with mock.patch.object(jax, 'devices', return_value=jax.devices()[:1]):
  ...
  def test_construct_restore_args_with_fallback_sharding(self):
    simple_sharding = jax.sharding.NamedSharding(
    ...
    @flax.struct.dataclass
    class TestTree(flax.struct.PyTreeNode):
      a: Any
      b: Any
      c: Any
      d: Any
    ...
    target_tree = TestTree(
        a=jax.ShapeDtypeStruct((), jnp.int32),
        b=jax.ShapeDtypeStruct((2, 4), jnp.float64, sharding=None),
        c=jax.ShapeDtypeStruct((2, 4), jnp.float64, sharding=None),
        d=jax.ShapeDtypeStruct((2, 4), jnp.float64, sharding=simple_sharding),
        e=jax.ShapeDtypeStruct((2, 4), jnp.float64, sharding=None),
    )

    metadata_sourced_sharding = jax.sharding.NamedSharding(
        mesh=jax.sharding.Mesh(jax.devices(), ('x',)),
        spec=jax.sharding.PartitionSpec(None, 'x'),
    )

    valid_sharding_metadata = (
        sharding_metadata.NamedShardingMetadata.from_jax_sharding(
            metadata_sourced_sharding
        )
    )
    ...
    # Test constucting restore args with a target tree.
    restore_args_target_tree = (
        standard_checkpoint_handler._construct_restore_args(
            target_tree, get_metadata_tree, fallback_sharding
        )
    )

    with self.subTest(name='target_tree_use_default_restore_args'):
      self.assertEqual(
          expected_restore_args_using_target_tree.a,
          restore_args_target_tree.a,
      )
    with self.subTest(name='target_tree_use_metadata_sharding'):
      self.assertEqual(
          expected_restore_args_using_target_tree.b,
          restore_args_target_tree.b,
      )
    with self.subTest(name='target_tree_use_fallback_sharding'):
      self.assertEqual(
          expected_restore_args_using_target_tree.c,
          restore_args_target_tree.c,
      )
    with self.subTest(name='target_tree_use_explicit_leaf_sharing'):
      self.assertEqual(
          expected_restore_args_using_target_tree.d,
          restore_args_target_tree.d,
      )
    with self.subTest(name='target_tree_use_metadata_valid_sharding'):
      self.assertEqual(
          expected_restore_args_using_target_tree.e,
          restore_args_target_tree.e,
      )
    ...
    restore_args_without_target_tree = (
        standard_checkpoint_handler._construct_restore_args(
            None, get_metadata_tree, fallback_sharding
        )
    )

    self.assertSameElements(
        expected_restore_args_without_target_tree.keys(),
        restore_args_without_target_tree.keys(),
    )
    with self.subTest(name='default_restore_args'):
      self.assertEqual(
          expected_restore_args_without_target_tree['a'],
          restore_args_without_target_tree['a'],
      )
      self.assertEqual(
          expected_restore_args_without_target_tree['b'],
          restore_args_without_target_tree['b'],
      )
      self.assertEqual(
          expected_restore_args_without_target_tree['d'],
          restore_args_without_target_tree['d'],
      )
      self.assertEqual(
          expected_restore_args_without_target_tree['e'],
          restore_args_without_target_tree['e'],
      )

    with self.subTest(name='use_fallback_sharding'):
      self.assertEqual(
          expected_restore_args_without_target_tree['c'],
          restore_args_without_target_tree['c'],
      )

    # Invalid metadata AND no fallback
    with self.assertRaisesRegex(ValueError, 'Topology mismatch detected'):
      standard_checkpoint_handler._construct_restore_args(
          target_tree, get_metadata_tree, fallback_sharding=None
      )


class StandardCheckpointHandlerTestJaxRandomKeys(
  ...
  def test_save_restore_random_keys_with_jax_eval_shape(self):
    ...
    abstract_pytree = jax.eval_shape(sharded_create_state_fn, jax.random.key(0))

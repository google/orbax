...
class PyTreeCheckpointHandlerTest(
  ...
  def get_expected_chunk_shape(
    ...
    return test_utils.get_expected_chunk_shape(arr)
  ...
  def validate_metadata(
    ...
    def _metadata(value):
      ...
      if isinstance(value, jax.Array):
        expected_chunk_shape = self.get_expected_chunk_shape(value)
        return value_metadata.ArrayMetadata(
            name='',
            directory=None,
            shape=value.shape,
  ...
  def test_save_sharding(self, use_ocdbt: bool):
    ...
    with (path / _SHARDING).open('r') as file:
      data = json.load(file)
  ...
  def test_save_strings(
    ...
    with self.ocdbt_checkpoint_handler(
      ...
      with (self.directory / '_strings.json').open() as file:
        data = json.load(file)
        self.assertCountEqual(
            data.keys(),
            {'foo', 'bar'},
  ...
  def test_aggregate_all(self, with_metadata: bool):
    ...
    if not path.exists():
      path = epath.Path(
          '/app/orbax_repo/checkpoint/orbax/checkpoint/testing/aggregate_all.checkpoint'
      )
  ...
  def test_aggregate_some(self, with_metadata: bool):
    ...
    if not path.exists():
      path = epath.Path(
          '/app/orbax_repo/checkpoint/orbax/checkpoint/testing/aggregate_some.checkpoint'
      )
    files = [p.name for p in path.iterdir()]
    self.assertSameElements(
        [
  ...
  @parameterized.product(use_ocdbt=(True, False))
  def test_save_restore_random_keys_with_jax_eval_shape(self, use_ocdbt: bool):
    # TODO(b/393160483) investigate Pathways remote Python support for
    # random.keys.
    if utils.is_pathways_backend():
      self.skipTest(
    ...
    with self.ocdbt_checkpoint_handler(
      ...
      restored = restore_handler.restore(
          self.directory, args=PyTreeRestoreArgs(abstract_pytree)
      )
      test_utils.assert_tree_equal(self, pytree, restored)
  ...
  @parameterized.product(use_ocdbt=(True, False), sharded_pytree=(True, False))
  ...
  @parameterized.product(use_ocdbt=(True, False))
  def test_partial_restore_with_omission_no_restore_args(self, use_ocdbt: bool):
    """Basic save and restore test."""
    directory = self.directory / 'partial_restore'
    directory.mkdir(parents=True, exist_ok=True)
  ...
  @parameterized.product(use_ocdbt=(True, False))
  def test_partial_restore_with_omission_empty_container(self, use_ocdbt: bool):
    """Basic save and restore test."""
    directory = self.directory / 'partial_restore'
    directory.mkdir(parents=True, exist_ok=True)
  ...
  def test_partial_restore_with_placeholder_unexpected_keys(
    ...
    reference_item = {
        ...
        'c': {
            'a': 0,
            'e': PLACEHOLDER,
            'f': PLACEHOLDER,  # Unexpected key.
        },
        'z': PLACEHOLDER,  # Unexpected key.
    }
    expected = {
        'a': self.pytree['a'],
        'b': PLACEHOLDER,
    ...
    restore_args['c']['f'] = ArrayRestoreArgs()
    restore_args['z'] = ArrayRestoreArgs()
  ...
  def test_partial_restore_with_placeholder_unexpected_keys_no_placeholder(
    ...
    reference_item = {
        ...
        'z': 0,  # Unexpected key, but not a placeholder.
    }
    restore_args['z'] = ArrayRestoreArgs()
  ...
  def test_partial_restore_with_omission_unexpected_keys(
    ...
    reference_item = {
        ...
        'c': {
            'a': 0,
            'f': PLACEHOLDER if use_placeholder else 0,  # Unexpected key.
        },
        'z': PLACEHOLDER if not use_placeholder else 0,  # Unexpected key.
    }
    expected = {
        'a': self.pytree['a'],
        'c': {
            'a': self.pytree['c']['a'],
            'f': PLACEHOLDER if use_placeholder else 0,
        },
        'z': PLACEHOLDER if not use_placeholder else 0,
    }
    restore_args['c']['f'] = ArrayRestoreArgs()
    restore_args['z'] = ArrayRestoreArgs()

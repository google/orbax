  ...
  match template:
    ...
    case mapping if isinstance(mapping, abc.Mapping):
      if utils.isinstance_of_namedtuple(structure):
        ...
        raise TypeError(
            f'Type mismatch at path {path}: template has type {type(template)},'
            f' but structure has type {type(structure)}.'
        )
    ...
    case sequence if isinstance(sequence, abc.Sequence) and not isinstance(
      ...
      if len(structure) != len(template):
        raise ValueError(
            f'Length mismatch at path {path}: template has length'
    ...
    case v if utils.is_leaf_node(v):
      ...
      return structure
    case _:
      raise TypeError(f'{path}: Unknown internal node type {type(structure)}.')

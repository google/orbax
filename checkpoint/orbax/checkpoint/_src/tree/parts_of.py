...
Not to be confused with `jax.tree_utils.Partial`, which does partial function
application.
"""
...
class PartsOf(Generic[T]):
  ...
  _template: jax.tree_util.PyTreeDef
  ...
  def __init__(
    ...
    unused_leaves, self._template = jax.tree_util.tree_flatten(template)
    self._present = {
        k: v
  ...
  @classmethod
  def from_structure(cls, structure: T) -> 'PartsOf[T]':
  ...
  def _with_present_values(
      cls,
      template: jax.tree_util.PyTreeDef,
  ...
  def overlay(
    ...
    Args:
      other: The partial tree to overlay. Must have the same template as this
        one.
      strict: If True, the overlay operation will fail if sets of values are not
        disjoint in this tree and `other`.
  ...
  def tree_flatten(self) -> tuple[tuple[T, ...], jax.tree_util.PyTreeDef]:
  ...
  def tree_unflatten(
      cls,
      aux_data: jax.tree_util.PyTreeDef,
...
jax.tree_util.register_pytree_node_class(PartsOf)
...
def map_leaves(
  ...
  Args:
    f: The function to apply to each leaf of the structure. It must accept as
      many arguments as given here, any number of which may be `PLACEHOLDER`. It
      must return an appropriate non-`None` leaf value, or `PLACEHOLDER`.
...
def map_leaves_with_path(
  ...
  PartsOf-compatible equivalent of `jax.tree_util.tree_map_with_path`.
...
def value_key_from_path(path: tuple[Any, ...]) -> tuple[Any, ...]:
  ...
  Args:
    path: a JAX tree path into a PartsOf object (i.e. obtained via one of
      `jax.tree_util.*` methods on a PartsOf pytree).

...
def tree_difference(
    ...
    b,
    is_leaf: Optional[Callable[[Any], bool]] = None,
    leaves_equal: Callable[[Any, Any], bool] | None = None,
):
  ...
  if isinstance_of_namedtuple(a):
    diffs = type(a)(*(
        tree_difference(aa, bb, is_leaf, leaves_equal) for aa, bb in zip(a, b)
    ))
    ...
    diffs = {
        k: diff
        for k, diff in (
            (k, tree_difference(a[k], b[k], is_leaf, leaves_equal))
            for k in a
            if k in b
        )
        if diff is not None

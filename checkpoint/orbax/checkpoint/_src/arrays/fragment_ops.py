...
"""Operations for the merge and construction of fragments."""
...
def _rank_0_fragment(fragment_t: type[F]) -> _HashedByIndex[F]:
  if fragment_t is AbstractFragment:
    return _RANK_0_ABSTRACT
  elif fragment_t is NpFragment:
    return _HashedByIndex(NpFragment(index=(), value=np.array(0)))
  else:
    return _HashedByIndex(JaxFragment(index=(), value=jnp.array(0)))


def gen_overlap_fragments(
...
def extract_fragment(
  ...
  overlaps = [
      *gen_overlap_fragments(fragment_t, source_fragments, required_fragment)
  ]
...
def _union_fragments(
  ...
  for x, edge_group in itertools.groupby(edges, key=get_x):
    ...
    for out_fragment, start in out.items():
      if start != x:
...
def union_fragment_list(
  ...
  for f in _union_fragments(
    ...
    if concrete:
      yield extract_fragment(fragment_t, fs, f.fragment)
    else:
      yield f.fragment

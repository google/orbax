...
def _override_ocdbt_kvspec_parameters_for_temporary_metadata(
  ...
  if write_mode != OcdbtWriteMode.COMMIT_TEMPORARY:
    manifest_spec = (
        f'file://{temporary_metadata_context.path}'
        f'/{_OCDBT_TMP_METADATA_PREFIX}'
    )
  ...
  base_driver_spec = {
      ...
      'layers': [
          ...
          # metadata-related files are routed to the temporary directory.
          {
              ...
              'base': f'file://{temporary_metadata_context.path}/',
          },
      ],
  }
...
def _build_ocdbt_kvstore_tspec(
  ...
  if is_gcs_path:
    ...
    trailing_slash = '/' if temporary_metadata_context is not None else ''
    base_driver_spec = {
        'driver': DEFAULT_DRIVER,
        'path': str(directory) + trailing_slash,
    }

...
def _all_backend_devices() -> tuple[jax.Device, ...]:
  ...
  if local_devices:
    for device in local_devices[0].client._get_all_devices():  # pylint: disable=protected-access
      key = (id(device.client), device.id, _device_platform(device))
      if key not in seen:
        seen.add(key)
...
def _get_cpu_device_map() -> dict[int, jax.Device]:
  ...
  That keeps both lookups working:
  - `120 -> CpuDevice(id=0)[PjRtIFRTDeviceId=120]` for controller-serialized
    specialization payloads
  - `0 -> CpuDevice(id=0)[PjRtIFRTDeviceId=120]` for worker-local callers that
    only ever knew the backend's local id namespace
  """
  cpu_device_map: dict[int, jax.Device] = {}
  for device in _all_backend_devices():
  ...
  for device in _all_backend_devices():
    if _device_platform(device) != 'cpu':
      continue
    cpu_device_map.setdefault(device.id, device)
...
def _normalize_mesh_to_colocated_cpu(
  ...
  if not devices or all(
      _device_platform(device) == 'cpu' for device in devices
  ):
    return mesh
  cpu_devices = np.vectorize(_to_serializable_cpu_device, otypes=[object])(
      mesh.devices
  )
...
def install_pathways_colocated_serialization_patch() -> None:
  """Installs a Pathways-safe colocated-python serialization patch.

  Pathways failures happened while JAX is pickling and unpickling
  callable specializations that contain mesh-backed shardings.
  ...
  def _orbax_reduce_mesh(mesh: jax.sharding.Mesh):
    return original_reduce_mesh(_normalize_mesh_to_colocated_cpu(mesh))

  def _orbax_reduce_device_list(device_list: cp_serialization.DeviceList):
...
def to_colocated_python(input_tree: PyTree) -> PyTree:
  ...
  def _get_sharding(x):
    if isinstance(x, jax.Array):
      return colocated_cpu_sharding(x.sharding)
    return None
...
def make_scalar_array_like(
  ...
  Args:
    ...
    like: An existing array to match the shape and sharding against.
    dtype: The data type of the resulting array.

  Returns:
    A new jax.Array with the specified value, matching the target sharding.
  """
...
def make_array_placeholder(spec: jax.ShapeDtypeStruct) -> jax.Array:
  """Builds a zero-valued array matching the given spec.

  This avoids triggering global allocations.

  Args:
    spec: The shape and dtype specification for the array.

  Returns:
    A zero-filled jax.Array matching the provided specification.
  """

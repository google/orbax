...
class ColocatedTransportTest(absltest.TestCase):
  ...
  def test_get_cpu_device_map_prefers_ifrt_global_ids(self):
    ...
    conflicting_local_id_device = mock.Mock(
        id=34, platform='cpu', client=object()
    )
    conflicting_local_id_device.__repr__ = mock.Mock(
        return_value='CpuDevice(id=34)'
    )

    with mock.patch.object(
        colocated_transport,
        '_all_backend_devices',
        return_value=(local_device, conflicting_local_id_device),
    ):

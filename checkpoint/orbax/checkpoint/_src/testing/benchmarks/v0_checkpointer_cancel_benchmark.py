...
class V0CheckpointerCancelBenchmark(benchmarks_core.BenchmarksGenerator):
  ...
  def test_fn(
    ...
    if options.perform_cancellation:
      assert step not in mngr.all_steps(), (
          f'Expected step {step} to be cancelled and not present in all_steps:'
          f' {mngr.all_steps()}'
      )
      assert mngr.latest_step() is None, (
          f'Expected latest step to be None after cancellation, got'
          f' {mngr.latest_step()}'
      )
      logging.info(
          'Successfully verified that cancelled step %d was not saved.', step
      )
    else:
      assert step in mngr.all_steps(), (
          f'Expected step {step} to be saved in all_steps: {mngr.all_steps()}'
      )
      assert mngr.latest_step() == step, (
          f'Expected latest step to be {step}, got {mngr.latest_step()}'
      )
      logging.info('Restoring checkpoint at step %d to verify integrity.', step)
      with metrics.measure('restore_cost'):
        restored = mngr.restore(step, args=ocp.args.StandardRestore(pytree))
      logging.info('Finished restoring checkpoint at step %d.', step)
      with metrics.measure('correctness_check'):
        pytree_utils.assert_pytree_equal(pytree, restored)
      logging.info(
          'Successfully verified checkpoint integrity for step %d.', step
      )

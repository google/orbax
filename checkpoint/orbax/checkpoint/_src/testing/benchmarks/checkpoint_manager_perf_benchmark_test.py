...
class CheckpointManagerPerfBenchmarkTest(parameterized.TestCase):
  ...
  def test_benchmark_test_fn(self, use_ocdbt, use_zarr3):
    ...
    mock_wait = self.mock_checkpoint_manager.return_value.wait_until_finished
    ...
    self.assertContainsSubset(
        {'save_0', 'train_step_0', 'wait_until_finished', 'restore'},
        result.metrics.timings.keys(),
    )
    self.assertEqual(mock_save.call_count, 200)
    ...
    self.assertIsNotNone(restore_args['pytree_item'].restore_args)

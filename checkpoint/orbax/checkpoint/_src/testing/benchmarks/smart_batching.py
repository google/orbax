...
_MAX_BATCH_SIZE_MB = flags.DEFINE_integer(
    'max_batch_size_mb', 10, 'Maximum size of a single batch in GB.'
)
...
async def _execute_batch_async(
  ...
  load_start_time = time.time()
  _log_info('\033[1mProcessing batch %s/%s...\033[0m', batch_index, num_batches)
  _log_info(
      '[Batch %d] Loading into Host RAM from Safetensors, starting at %.2f'
      ' seconds, total tensors: %d | Batch Size: %.2f MB',
      batch_index,
      load_start_time,
      len(abstract_pytree),
      batch_size,
  ...
  for path, abstract_pytree in file_abstract_trees.items():
    _log_info(
        '[Batch %d] Loading path %s with %d tensors',
        batch_index,
        path,
        len(abstract_pytree),
    )
    ...
    _log_info(
        '[Batch %d] Loaded path %s with %d tensors | Load Size: %.2f MB | Load'
        ' Throughput: %.2f MB/s',
        batch_index,
        path,
  ...
  _log_info(
      '[Batch %d] Load Time: %.2f seconds | Load Size: %.2f MB | Load'
      ' Throughput: %.2f MB/s',
      batch_index,
      load_end_time - load_start_time,
...
async def _load_safetensors(
  ...
  with ocp_v1.Context(
    ...
    # batches, batch_sizes = await (
    #     safetensors_layout.create_loading_plan_by_file(
    #         paths, abstract_pytree
    #     )
    # )
...
async def run_cpu_batching(
  ...
  load_end_time = time.time()
  _log_info(
      'Total Loading Time: %.2f seconds | Total Load Size: %.2f MB | Total Load'
      ' Throughput: %.2f MB/s',
      load_end_time - load_start_time,
      total_load_size,

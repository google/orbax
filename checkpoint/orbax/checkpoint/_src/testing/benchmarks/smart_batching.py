...
_MAX_BATCH_SIZE_GB = flags.DEFINE_integer(
    'max_batch_size_gb', 10, 'Maximum size of a single batch in GB.'
)
  ...
  with ocp_v1.Context(
      ...
      array_options=ocp_v1.options.ArrayOptions(
          loading=ocp_v1.options.ArrayOptions.Loading(
              concurrent_bytes=_MAX_BATCH_SIZE_GB.value * 1024**3,
          ),
      ),
    ...
    loaded_pytree = await layout.load_pytree(input_path)
  total_load_size = sum(
      leaf.nbytes
      for leaf in jax.tree_util.tree_leaves(loaded_pytree)

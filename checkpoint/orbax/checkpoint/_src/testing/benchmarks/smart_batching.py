...
async def verify_local_integrity(
  ...
  for path, _ in flat_meta:
    ...
    if val_src.dtype != val_dst.dtype:
      _log_error(
          '❌ DTYPE MISMATCH: %s | Src %s != Dst %s',
          tensor_name,
          val_src.dtype,
          val_dst.dtype,
      )
...
async def get_tensor_to_path_indexing(paths: Sequence[Path]):
  ...
  if not await async_path.exists(path_):
    for path in paths:
      header, _ = await safetensors_layout._read_safetensors_header(path)  # pylint: disable=protected-access
      for name in header:
...
async def _execute_batch_async(
    abstract_pytree: dict[str, Any],
    batch_index: int,
    num_batches: int,
    batch_size: float,
    tensor_to_path: dict[str, Path],
) -> Tuple[float, int]:
  """Loads and saves a single batch."""
  load_start_time = time.time()
  _log_info(
      '\033[1m[VM %d] Processing batch %s/%s...\033[0m',
      jax.process_index(),
      batch_index + 1,
      num_batches,
  )
  _log_info(
      ...
      ' %.2f seconds, total tensors: %d | Batch Size: %.2f MB',
      jax.process_index(),
      batch_index + 1,
      load_start_time,
      len(abstract_pytree),
  ...
  for tensor_name in abstract_pytree.keys():
  ...
  for path, abstract_pytree in file_abstract_trees.items():
    ...
    with ocp_v1.Context(
        multiprocessing_options=ocp_v1.options.MultiprocessingOptions(
            primary_host=_WORKERS.value,
        ),
    ):
      current_loaded_tensors = await layout.load_pytree(
          path=path, abstract_pytree=abstract_pytree
      )
    if inspect.iscoroutine(current_loaded_tensors):
      current_loaded_tensors = await current_loaded_tensors
    current_loaded_tensors = cast(Dict[str, Any], current_loaded_tensors)
    all_loaded_tensors.update(current_loaded_tensors)
    current_load_size = sum(t.nbytes for t in current_loaded_tensors.values())
    _log_info(
        '[VM %d] [Batch %d] Loaded path %s with %d tensors | Load Size: %.2f MB'
        ...
        current_load_size / (1024 * 1024) / (time.time() - load_start_time),
    )
  load_end_time = time.time()
  total_load_size = sum(t.nbytes for t in all_loaded_tensors.values())
  _log_info(
      '[VM %d] [Batch %d] Load Time: %.2f seconds | Load Size: %.2f MB | Load'
      ' Throughput: %.2f MB/s',
      jax.process_index(),
      batch_index + 1,
      load_end_time - load_start_time,
      total_load_size / (1024 * 1024),
      total_load_size / (1024 * 1024) / (load_end_time - load_start_time),
  )
  ...
  if _SAVING_ENABLED.value:
    ...
    ocp_v1.partial.save_pytree(output_dir, all_loaded_tensors)
  ...
  del all_loaded_tensors
  return total_save_time, batch_index
...
async def _load_safetensors(
  ...
  if await async_path.is_dir(path):
    paths = await async_path.glob(path, f'*{".safetensors"}')
    paths = sorted(paths)

  tensor_to_path = await get_tensor_to_path_indexing(paths)
  with ocp_v1.Context(
      array_options=ocp_v1.options.ArrayOptions(
          loading=ocp_v1.options.ArrayOptions.Loading(
              concurrent_bytes=_MAX_BATCH_SIZE_MB.value * 1024**2,
          ),
      ),
  ):
    batches, batch_sizes = await safetensors_layout.create_loading_plan(
  ...
  # --- SHARDING LOGIC ---
  # Each VM gets a subset of the total batches (Round Robin)
  process_idx = jax.process_index()
  process_cnt = jax.process_count()

  my_batches = []
  my_batch_sizes = []
  my_indices = []
  for i, (b, s) in enumerate(zip(batches, batch_sizes)):
    if i % process_cnt == process_idx:
      my_batches.append(b)
      my_batch_sizes.append(s)
      my_indices.append(i)

  batches = my_batches
  batch_sizes = my_batch_sizes

  _log_info(
      'VM %d/%d assigned %d batches out of %d total.',
      process_idx,
      process_cnt,
      len(batches),
      total_batches_count,
  ...
  with ThreadPoolExecutor(max_workers=max_threads) as pool:
    ...
    _log_info(
        '\033[1m[VM %d] Final load size: %.2f MB | Final load time: %.2f'
        ' seconds | Final load throughput: %.2f MB/s \033[0m',
        jax.process_index(),
        total_load_size,
        time.time() - start_time,
        (total_load_size / (time.time() - start_time)),
    )
    return total_load_size
...
async def run_cpu_batching(
  ...
  if jax.process_index() == 0:
    ...
    if gfile.exists(partial_save_dir):
      _log_info(
          'Removing existing partial save directory: %s', partial_save_dir
      )
  ...
  _log_info('step 5: Time to load using Safetensors Flax API')
  _log_info('step 6: Verifying Local Integrity')
  ...
  load_start_time = time.time()
  total_load_size = await _load_safetensors(input_path)
  load_end_time = time.time()
  # _log_info(
  #     'Total Loading Time: %.2f seconds | Total Load Size: %.2f MB |'
  #     ' Total Load Throughput: %.2f MB/s',
  #     load_end_time - load_start_time,
  #     total_load_size,
  ...
  if _SAVING_ENABLED.value:
    ...
    if jax.process_index() == 0:
      _log_info(
          '------------ Step 4: Finalizing Native Orbax Checkpoint'
          ' --------------'
      )
      save_start_time = time.time()
      ...
      _log_info(
          'Finalizing Native Orbax Checkpoint Time: %.2f seconds',
          total_save_time,
      )
  ...
  if _OFFICIAL_LOAD_ENABLED.value:
    ...
    _log_info(
        '------------ Step 5:  Time to load using Safetensors Flax API'
        ' ------------\n'
    )
...
def main(argv: Sequence[str]) -> None:
  ...
  # Initialize JAX Distributed to allow xpk multi-node coordination

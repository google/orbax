...
async def _execute_batch_async(
  ...
  load_start_time = time.time()
  _log_info(
      'Loading batch into Host RAM from Safetensors, starting at %.2f seconds',
      load_start_time,
  )
...
async def run_cpu_batching(
  ...
  with ThreadPoolExecutor(max_workers=_WORKERS.value) as pool:
    ...
    for r in results:
      ...
      print(
          f'Read Batch {batch_index} with {batch_size / (1024 * 1024)} MB in'
          f' {load_time} sec (throughput:'
          f' {batch_size / (1024 * 1024) / load_time} MB/sec)'
      )

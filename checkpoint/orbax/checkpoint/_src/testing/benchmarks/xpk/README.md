...
| `--ramdisk_directory` | `None` | Mount a ramdisk at this path (e.g., `/tmp/ramdisk`). If set, `--mtc_enabled` must also be True. |
| `--mtc_enabled` | `False` | If True, enable MTC for the workload. Required if using `--ramdisk_directory` for ECM testing. |
...
The Emergency Checkpoint Manager (ECM) workflow can be tested using MTC
(Multi-host TPU Checkpointing) specific flags. This involves saving checkpoints
to a local RAM disk (`gcsfuse`-backed) that persists across process restarts.

Key flags for ECM testing:

*   `--mtc_enabled`: Enables MTC, mounting a persistent RAM disk for
    checkpoints via `gcsfuse` and Bucket CSI driver. You must use a Hierarchical
    Namespace (HNS) enabled bucket for MTC (e.g., `gs://orbax-benchmarks-hns`).
    The Bucket CSI driver configuration is managed by `cpc.yaml`.
...
MTC requires a GCS bucket with Hierarchical Namespace (HNS) enabled. If you
don't have one, create one via Pantheon or `gcloud`. You must use this bucket
in `--output_directory` when `--mtc_enabled` is True.

Additionally, you need to grant specific IAM permissions to the bucket for MTC
to work correctly.
...
```bash
python3 launch_xpk.py -- \
  --cluster_name orbax-cluster-mtc \
  --tpu_type v5litepod-16 \
  --zone us-west1-c \

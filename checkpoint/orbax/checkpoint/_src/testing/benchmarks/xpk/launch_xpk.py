...
def construct_workload_command(
  ...
  if enable_pathways:
    ...
    if hardware_type == HardwareType.TPU:
      ...
      env_vars = [
          'export megascale_port=%port_megascale%',
          'export JAX_PLATFORMS=cpu',
          'export JAX_NUM_PROCESSES=$JAX_PROCESS_COUNT',
          # Calculate the true global rank:
          # (Job_Index * Pods_Per_Job) + Pod_Index
          (
  ...
  if hardware_type == HardwareType.CPU:
    python_cmd += ' --jax_cpu_collectives_implementation=megascale'
  if enable_pathways:
    python_cmd = (

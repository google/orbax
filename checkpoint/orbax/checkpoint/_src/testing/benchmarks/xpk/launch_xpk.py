...
_CONFIG_DIRECTORY = flags.DEFINE_string(
    'config_directory',
    None,
    'Remote directory for configuration file upload. Defaults to'
    ' --output_directory.',
)
...
_SKIP_XPK_VALIDATION = flags.DEFINE_boolean(
    'skip_xpk_validation',
    False,
    'If True, skip XPK validation.',
)
...
def construct_xpk_command(
  ...
  if _SKIP_XPK_VALIDATION.value:
    base_cmd.insert(3, '--skip-validation')
...
def upload_config(local_path: str, remote_root: str, run_id: str) -> str:
  """Uploads the local config file to remote and returns the path."""
  filename = os.path.basename(local_path)
  remote_path = os.path.join(remote_root, run_id, filename)

  if remote_path.startswith('gs://'):
    Console.print_info(f'Uploading config to {remote_path}')
    run_command(
        ['gcloud', 'storage', 'cp', local_path, remote_path],
        suppress_output=True,
    )
  elif _STORAGE.value and remote_path.startswith('/lustre'):
    Console.print_warning(
        f'Skipping local upload for Lustre path: {remote_path}. '
        'Ensure the config is available at this path inside the pod.'
    )
  else:
    Console.print_info(f'Uploading config to {remote_path}')
    # Handle local paths (assuming mounted if not gs://)
    os.makedirs(os.path.dirname(remote_path), exist_ok=True)
    run_command(['cp', local_path, remote_path], suppress_output=True)
  return remote_path
  ...
  if mount_csi_driver:
    run_command(
        [
            ...
            '-c',
            (
                f'BUCKET_NAME={bucket} envsubst < {cpc_yaml_path} | kubectl'
                ' apply -f -'
            ),
        ],
        suppress_output=not _VERBOSE.value,
...
def main(argv: Sequence[str]) -> None:
  ...
  if _WORKLOAD_NAME.value is not None:
    ...
    if _ENABLE_PATHWAYS.value:
      ...
      workload_name_base = f'orbax-{base_name}-{timestamp}'.replace(
          '_', '-'
      ).lower()
  ...
  remote_config_path = upload_config(_CONFIG_FILE.value, config_root, run_id)
  ...
  for i in range(attempts):
    ...
    try:
      ...
      Console.print_success(f'Workload {workload_name} completed successfully.')

...
class MetricsManager:
  ...
  def _write_per_host_means_sidecar(
    ...
    Filesystem-based gather: sidecars avoid jax.distributed allgather (Gloo is
    flaky around process shutdown in the docker harness) and work on any shared
    filesystem. Every host writes even with no successful metrics so all hosts
    reach the caller's barrier; an empty sidecar is harmless.
  ...
  def _write_summary_cards(
    ...
    if self._enable_per_host_metrics:
      writer = metric_writers.create_default_writer(
          self._tensorboard_dir, collection=f"{benchmark_name}/__summary__"
      )
      owned = True
    else:
      writer = self._get_writer(benchmark_name)

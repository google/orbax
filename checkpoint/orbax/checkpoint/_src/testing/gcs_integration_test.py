...
class GcsIntegrationTest(parameterized.TestCase):
  ...
  def test_save_restore_ocdbt_gcs(
      self,
      tree: dict[str, Any],
      step: int,
  ):
    ...
    logging.info('Using GCS path for test: %s', path)
    ...
    logging.info('Max object size for %s test: %s', self.id(), max_size)
    ...
    logging.info('%s save/restore to GCS successful.', self.id())
    logging.info('GCS integration test passed. Checkpoint data is in %s.', path)
    logging.info(
        'Run `gcloud storage ls -r -l %s` to view checkpoint files.', path
    )
  ...
  def test_benchmark_different_target_sizes(
      self,
      name: str,
      target_size: int,
  ):
    ...
    logging.info('Starting benchmark for target size: %s', name)
    path = (
        epath.Path(_TEST_GCS_DIR.value)
    ...
    max_size = get_max_object_size(path)
    logging.info('Max object size for %s test: %s', name, max_size)
    ...
    logging.info(
        'Target size: %s, save time: %.2fs, restore time: %.2fs',
        name,
        save_time,
        restore_time,
    )

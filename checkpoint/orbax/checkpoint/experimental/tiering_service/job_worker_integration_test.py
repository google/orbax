...
class RandomStream:
  ...
  def read(self, amt: int = -1) -> bytes:
    ...
    if amt is None or amt == -1:
      amt = min(remaining, self.chunk_size)
    else:
      amt = min(amt, remaining, self.chunk_size)
    data = self.rng.randbytes(amt)
...
class GcpLustreClientIntegrationTest(unittest.IsolatedAsyncioTestCase):
  ...
  async def asyncSetUp(self):
    ...
    self.import_client = gcp_storage_client.GcsToLustreClient(
        project=self.project,
        zone=self.location,
        instance=self.instance,
    )
    self.export_client = gcp_storage_client.LustreToGcsClient(
        project=self.project,
        zone=self.location,
        instance=self.instance,
    )
  ...
  async def test_import_and_export_lifecycle(self):
    ...
    try:
      ...
      while True:
        if time.perf_counter() - poll_start > 600:
          self.fail("Import operation timed out after 600 seconds")
      ...
      while True:
        if time.perf_counter() - poll_start > 600:
          self.fail("Export operation timed out after 600 seconds")
...
class GcsToGcsClientIntegrationTest(unittest.IsolatedAsyncioTestCase):
  ...
  async def test_gcs_to_gcs_lifecycle(self):
    ...
    try:
      ...
      while True:
        if time.perf_counter() - poll_start > 600:
          self.fail("GCS-to-GCS transfer timed out after 600 seconds")
        result = await self.client.poll_operation(req_id)
        logging.info(

...
"""Integration tests for GcpLustreClient.

These tests run against an existing GCP Managed Lustre cluster and GCS bucket.
To execute these tests, set the following environment variables:
  ...
  CTS_TEST_GCS_BUCKET: The GCS bucket (e.g. "gs://my-bucket").
...
class RandomStream:
  ...
  def __init__(self, total_size: int, chunk_size: int = 8 * 1024 * 1024):
    self.google_crc32c = importlib.import_module("google_crc32c")
...
class GcpLustreClientIntegrationTest(unittest.IsolatedAsyncioTestCase):
  ...
  async def asyncSetUp(self):
    ...
    self.gcs_bucket = os.environ.get("CTS_TEST_GCS_BUCKET")
    ...
    if not all([self.location, self.instance, self.gcs_bucket]):
      self.skipTest(
          ...
          "CTS_TEST_GCS_BUCKET environment variables to run integration tests."
      )
  ...
  async def test_import_and_export_lifecycle(self):
    ...
    try:
      ...
      start_time = time.perf_counter()
      import_op = await self.client.trigger_import(
      ...
      while True:
        result = await self.client.poll_operation(import_op)
        logging.info("Import LRO %s status: %s", import_op, result.status)
        if result.status == job_worker.Status.COMPLETED:
          if "error" in result.detail_info:
            self.fail(f"Import operation failed: {result.detail_info['error']}")
          break
      ...
      start_time = time.perf_counter()
      export_op = await self.client.trigger_export(
          request_id=str(uuid.uuid4()),
      ...
      while True:
        result = await self.client.poll_operation(export_op)
        logging.info("Export LRO %s status: %s", export_op, result.status)
        if result.status == job_worker.Status.COMPLETED:
          if "error" in result.detail_info:
            self.fail(f"Export operation failed: {result.detail_info['error']}")
          break
        await asyncio.sleep(5)

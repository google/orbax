...
class CtsClientCli:
  ...
  def reserve(
    ...
    async def _run():
      ...
      try:
        logging.info(
            "Reserving asset path: %s (user=%s, auto_finalize=%s)",
            path,
            user,
            auto_finalize,
        )
        ...
        logging.info(
            "Reserve succeeded for path '%s': UUID=%s, Tier 0 Path=%s",
            path,
            uuid,
            t0_path,
        )
        ...
        if auto_finalize:
          logging.info("Auto-finalizing asset UUID: %s", uuid)
          await c.finalize(uuid)
          logging.info("Auto-finalization completed for asset UUID: %s", uuid)
          print(f"Auto-finalized asset UUID: {uuid}")
        else:
          logging.info(
              "Keep-alive task active for asset UUID: %s. Maintaining lease...",
              uuid,
          )
          ...
          logging.info("Finalizing asset UUID: %s", uuid)
          await c.finalize(uuid)
          logging.info("Finalization completed for asset UUID: %s", uuid)
  ...
  def finalize(self, uuid: str) -> None:
    ...
    async def _run():
      c = client.TieringClient(self._server_address)
      try:
        logging.info("Initiating finalize for asset UUID: %s", uuid)
        await c.finalize(uuid)
        logging.info("Finalize succeeded for asset UUID: %s", uuid)
  ...
  def prefetch(self, path_or_uuid: str) -> None:
    ...
    async def _run():
      c = client.TieringClient(self._server_address)
      try:
        logging.info("Initiating prefetch for %s...", path_or_uuid)
        print(f"Initiating prefetch for {path_or_uuid}...")
        t0_path = await c.prefetch(**self._get_id_kwargs(path_or_uuid))
        logging.info(
            "Prefetch resolved successfully for %s! Tier 0 path: %s",
            path_or_uuid,
            t0_path,
        )
        ...
        logging.info(
            "Prefetch lease active for %s. Maintaining lease...", path_or_uuid
        )
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(
            None,
        ...
        if is_uuid:
          ...
          if assets_list:
            ...
            logging.warning(
                "Could not retrieve asset UUID to release for path: %s",
                path_or_uuid,
            )
            print("Could not retrieve asset UUID to release.")
            return
        logging.info("Releasing prefetch lease for asset UUID: %s", uuid)
        await c.release(uuid)
        logging.info("Released prefetch for asset UUID: %s", uuid)
  ...
  def delete(self, path_or_uuid: str) -> None:
    ...
    async def _run():
      c = client.TieringClient(self._server_address)
      try:
        logging.info("Initiating delete for %s...", path_or_uuid)
        await c.delete(**self._get_id_kwargs(path_or_uuid))
        logging.info("Delete job queued successfully for %s", path_or_uuid)
  ...
  def info(self, path_or_uuid: str) -> None:
    ...
    async def _run():
      c = client.TieringClient(self._server_address)
      try:
        logging.info("Querying info for %s...", path_or_uuid)
        assets_list = await c.info(**self._get_id_kwargs(path_or_uuid))
        if not assets_list:
          logging.warning("No matching assets found for %s.", path_or_uuid)
          print("No matching assets found.")
          return
        logging.info(
            "Found %d matching asset(s) for %s.", len(assets_list), path_or_uuid
        )
  ...
  def evict(self, path: str) -> None:
    ...
    """
    logging.info("Evicting path: %s", path)
    ...
    if os.path.isdir(path):
      shutil.rmtree(path)
      logging.info("Evicted directory: %s", path)
      print(f"Evicted directory: {path}")
    else:
      os.remove(path)
      logging.info("Evicted file: %s", path)

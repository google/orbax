...
class LocalCheckpointDataValidatorTest(
  ...
  async def test_main(self, use_ocdbt: bool, use_zarr3: bool):
    ...
    devices_indices_map = array.sharding.devices_indices_map(array.shape)
    device_by_id = {d.id: d for d in jax.devices()}
    device_ids_by_process = multihost.distributed_to_device_ids()
    chunks_by_process = [
        tuple(
            index_to_chunk_id(
                devices_indices_map[device_by_id[d_id]],
                array.shape,
                array.sharding.shard_shape(array.shape),
            )
            for d_id in device_ids
        )
        for device_ids in device_ids_by_process
    ]

    if multihost.process_index() == 0:
      self.assertSameElements(present_chunk_ids, chunks_by_process[1])
      self.assertSameElements(missing_chunk_ids, chunks_by_process[0])
    elif multihost.process_index() == 1:
      self.assertSameElements(present_chunk_ids, chunks_by_process[0])
      self.assertSameElements(missing_chunk_ids, chunks_by_process[1])
    elif multihost.process_index() == 2:
      self.assertSameElements(present_chunk_ids, chunks_by_process[2])
      self.assertEmpty(missing_chunk_ids)
    elif multihost.process_index() == 3:
      self.assertSameElements(present_chunk_ids, chunks_by_process[3])
      self.assertEmpty(missing_chunk_ids)

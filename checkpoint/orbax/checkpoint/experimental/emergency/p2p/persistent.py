...
class PersistentCheckpointManager:
  ...
  def save(
    ...
    if self._in_primary_slice:
      return self._manager.save(step, args=args.state, force=force)
    return True

  def restore(self, step: int, args: args_lib.Composite) -> Any:
    ...
    """
    logging.info('Restoring step %s from persistent storage...', step)
    ...
    state = self._manager.restore(step, args=restore_args_obj)

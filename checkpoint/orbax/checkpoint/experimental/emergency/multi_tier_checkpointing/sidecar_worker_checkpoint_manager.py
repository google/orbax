...
def _check_unique_ids(name: str, ids: tuple[int, ...]) -> None:
  ...
  if duplicates:
    raise ValueError(f'{name} must be unique. Duplicate ids: {duplicates}.')


def _replicated_sharding_like(
...
class WorkerCheckpointManagerRaw:
  ...
  def __init__(
    ...
    if distributed_to_device_ids is not None:
      ...
      local_id_by_controller_id = {
          int(controller_id): int(device.id)
          for controller_id, device in zip(mesh_device_ids, cpu_devices)
      }
      missing_controller_ids = sorted({
          int(device_id)
          for device_id in itertools.chain.from_iterable(
              distributed_to_device_ids
          )
          if int(device_id) not in local_id_by_controller_id
      })
      if missing_controller_ids:
        raise ValueError(
            'distributed_to_device_ids contains CPU ids not present in '
            f'mesh_device_ids: {missing_controller_ids}.'
        )
      distributed_to_device_ids_fn = lambda: [
          [
              local_id_by_controller_id[int(device_id)]
              for device_id in device_ids
          ]
          for device_ids in distributed_to_device_ids
      ]
  ...
  def save(
    ...
    logging.info(
        'Pathways colocated MTC sidecar save step=%s: step found.',
        step,
    )
  ...
  def all_steps(self, dummy_array: jax.Array) -> jax.Array:
    ...
    return _array_on_sharding(
        ...
        _replicated_sharding_like(dummy_array.sharding),
    )

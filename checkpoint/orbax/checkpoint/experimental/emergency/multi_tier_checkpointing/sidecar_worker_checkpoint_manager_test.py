...
class SidecarWorkerCheckpointManagerTest(absltest.TestCase):
  ...
  def test_save_delegates_to_rcm_and_packs_result(self):
    ...
    step_array = jnp.asarray(7, dtype=jnp.int32)
  ...
  def test_should_save_delegates_to_rcm_and_packs_result(self):
    step_array = jnp.asarray(11, dtype=jnp.int32)
    packed_result = object()
  ...
  def test_restore_infer_uses_standard_worker_restore_inference_and_unwraps_state(
      self,
  ):
    ...
    result = self.manager.restore_infer(jnp.asarray(5, dtype=jnp.int32))
  ...
  def test_latest_step_returns_negative_one_when_no_step_exists(self):
    dummy_array = jnp.asarray(True)
    packed_result = object()
    ...
    self.assertIs(result, packed_result)
    mock_make_scalar.assert_called_once_with(-1, dummy_array, dtype=jnp.int32)
  ...
  def test_init_reconstructs_cpu_mesh_from_local_devices(self):
    ...
    with mock.patch.object(
        sidecar_lib.multihost,
        'is_jax_distributed_client_initialized',
        return_value=True,
    ), mock.patch.object(
        ...
        sidecar_lib,
        '_sc',
    ) as mock_signaling, mock.patch.object(
        sidecar_lib,
        'epath',
    ...
    mock_signaling.acquire_threadsafe_signaling_client_override.assert_not_called()
    ...
    self.assertNotIn('_ocdbt_process_id_override', mock_rcm.call_args.kwargs)

  def test_close_disables_threadsafe_signaling_override(self):
    dummy_array = jnp.asarray(True)
    packed_result = object()
    self.manager._threadsafe_signaling_override_enabled = True

    with mock.patch.object(
        sidecar_lib._sc, 'release_threadsafe_signaling_client_override'
    ) as mock_set_signaling, mock.patch.object(
        sidecar_lib.colocated_utils,
        'make_scalar_on_like',
        return_value=packed_result,
    ):
    ...
    mock_set_signaling.assert_called_once_with()

  def test_close_leaves_signaling_override_untouched_when_not_enabled(self):
    dummy_array = jnp.asarray(True)
    packed_result = object()
    self.manager._threadsafe_signaling_override_enabled = False
    ...
    self.assertIs(result, packed_result)
    self.manager._rcm.close.assert_called_once()
    mock_set_signaling.assert_not_called()

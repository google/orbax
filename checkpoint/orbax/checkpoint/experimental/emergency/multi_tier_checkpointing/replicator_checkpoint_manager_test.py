...
class ReplicatorLocalCheckpointEngineTest(parameterized.TestCase):
  ...
  def test_colocated_python_rejects_multi_controller_runtime(self):
    ...
    with mock.patch.object(
        replicator_checkpoint_manager.jax, 'process_count', return_value=2
    ):
      with self.assertRaisesRegex(ValueError, 'single-controller runtime'):
        ReplicatorCheckpointManager(

...
def _initialize_mtc_colocated(
  ...
  topology = pathways_topology.Topology.from_devices(
      tuple(all_devices), num_slices=num_slices
  )
  ...
  peer_ranks_by_worker_rank = tuple(
      ...
      for peer_ranks in topology.peer_ranks_by_worker_rank
  )
  logging.info(
  ...
  def _setup(dummy_arg: jax.Array, worker_rank_arg: jax.Array) -> jax.Array:
    ...
    _wait_for_replicator_file_to_disappear(
        loc_dir,
        timeout_seconds=min(
            timeout_seconds, _PATHWAYS_REPLICATOR_FILE_TIMEOUT_SECONDS
        ),
    )
...
def _block_and_process_restore_dir(
  ...
  for elapsed_seconds in range(timeout_seconds):
    ...
    for marker_path in marker_paths:
      ...
      # Treat it as a sentinel regardless of the marker artifact type.
      if step == '0':

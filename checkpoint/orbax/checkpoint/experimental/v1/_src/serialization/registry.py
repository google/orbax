...
class BaseLeafHandlerRegistry:
  ...
  def get(self, ty: Type[Any]) -> types.LeafHandler[Any, Any] | None:
  ...
  def get_abstract(
      self, abstract_ty: Type[Any]
  ) -> types.LeafHandler[Any, Any] | None:
  ...
  def get_all(
      self,
  ) -> Sequence[Tuple[Type[Any], Type[Any], types.LeafHandler[Any, Any]]]:
  ...
  def add(
      self,
      ty: Type[Any],
      abstract_ty: Type[Any],
      handler: types.LeafHandler[Any, Any],
  ):
    """Adds a handler for a given type."""
    current_handler = self.get(ty)
    handler_abstract_ty = self.get_abstract(abstract_ty)

...
class StringLeafHandler(types.LeafHandler[str, AbstractString]):
  ...
  async def _background_serialize(
    ...
    if not multihost.is_primary_host(
    ...
    async with cancellation.ignore_cancellation(
        'StringLeafHandler._background_serialize'
    ):
  ...
  async def _background_deserialize(
    ...
    async def _convert_to_string(tensorstore: ts.TensorStore):
      result = await tensorstore.read()
      return str(result)

    open_futures = [
        ts.open(
            self._get_json_tspec(
    ...
    return await asyncio.gather(*(_convert_to_string(t) for t in tensorstores))

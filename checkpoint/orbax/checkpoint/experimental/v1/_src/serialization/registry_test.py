...
class RegistryTest(absltest.TestCase):
  ...
  def test_simple_add_and_get(self):
    ...
    self.assertIsNone(reg.get(float))
    self.assertIsNone(reg.get_abstract(float))
    self.assertIsNone(reg.get_abstract(type(0.0)))
  ...
  def test_replace_with_different_abstract_type(self):
    ...
    reg.add(int, jnp.int64, int_handler2)

    self.assertIsNone(reg.get_abstract(int))

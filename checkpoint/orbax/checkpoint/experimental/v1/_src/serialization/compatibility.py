...
from orbax.checkpoint.experimental.v1._src.serialization import types as types_v1
...
) -> types_v1.SerializationParam[types_v1.Leaf]:
  return types_v1.SerializationParam(
...
def _construct_serialization_context(
    info: types_v0.ParamInfo,
) -> types_v1.SerializationContext:
  return types_v1.SerializationContext(
...
def _construct_deserialization_param(
...
) -> types_v1.DeserializationParam[array_leaf_handler.AbstractArray | None]:
  ...
  return types_v1.DeserializationParam(
      keypath=_keypath_from_param_name(info.name),
      value=value,
  )
...
def _construct_deserialization_context(
    info: types_v0.ParamInfo,
) -> types_v1.DeserializationContext:
  return types_v1.DeserializationContext(
...
def _validate_serialization_infos(
  ...
  info0 = infos[0]
  for info in infos[1:]:
    assert info0.parent_dir == info.parent_dir
    assert info0.ts_context == info.ts_context
    assert info0.byte_limiter == info.byte_limiter
...
def _validate_deserialization_infos(
  ...
  for info in infos:
    assert info0.parent_dir == info.parent_dir
    assert info0.is_ocdbt_checkpoint == info.is_ocdbt_checkpoint
    assert info0.use_zarr3 == info.use_zarr3
    assert info0.ts_context == info.ts_context
    assert info0.byte_limiter == info.byte_limiter
...
class CompatibleTypeHandler(
    types_v0.TypeHandler, Generic[types_v1.Leaf, types_v1.AbstractLeaf]
):
  """Compatibility wrapper to help leaf handlers to work as V0 type_handlers."""

  def __init__(
      self,
      leaf_handler: types_v1.LeafHandler[types_v1.Leaf, types_v1.AbstractLeaf],
      typestr: str,
  ):
  ...
  ) -> Sequence[types_v1.Leaf]:
    _validate_deserialization_infos(infos)

    params = []
    if args is None:
      args = [types_v0.RestoreArgs(restore_type=types_v1.Leaf)] * len(infos)
  ...
  def memory_size(
      self, values: Sequence[types_v1.Leaf]

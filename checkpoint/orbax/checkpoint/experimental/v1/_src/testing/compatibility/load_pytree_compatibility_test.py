...
class LoadPytreeCompatibilityTest(parameterized.TestCase):
  ...
  def test_load_pytree_non_critical_corruptions(self, version, alteration):
  ...
  def test_load_pytree_critical_corruptions(self, version, alteration):
    path = self.base_dir.joinpath(
        f'{version}_checkpoints',
        'composite_checkpoint',
  ...
  def test_load_incorrect_path(self, version):
    checkpoint_path = (
        self.base_dir
        / f'{version}_checkpoints'
        / 'composite_checkpoint'

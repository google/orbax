...
class PytreeMetadataCompatibilityTest(parameterized.TestCase):
  ...
  def setUp(self):
    ...
    self.expected_state_metadata = jax.tree.map(
        self._create_value_metadata, self.expected_state
    )
  ...
  def _create_value_metadata(self, value):
    ...
    if isinstance(value, jax.Array):
      sharding_metadata_obj = sharding_metadata.from_jax_sharding(
          value.sharding
      )
  ...
  def get_checkpoint_path(
      self, version, metadata_present, is_direct_checkpoint, is_pytree
  ):
  ...
  def setup_registry(
    ...
    if handler_registered:
      ...
      registry.add(ocp.handlers.PyTreeHandler, checkpointable_name=None, secondary_typestrs=[
      ...
      registry.add(ocp.handlers.JsonHandler, checkpointable_name=None, secondary_typestrs=[
  ...
  def _determine_expected_outcome(
    ...
    if version == 'v1':
      ...
      if not metadata_present:
        return True, InvalidLayoutError, (
  ...
  def test_pytree_metadata_compatibility(
    ...
    should_fail, error_type, error_msg = self._determine_expected_outcome(
        version,
        checkpointable_name,
        metadata_present,
        is_direct_checkpoint,
  ...
  def test_pytree_metadata_non_critical_corruptions(self, version, alteration):
    ...
    if alteration == 'missing_pytree_data_file__sharding':
  ...
  @parameterized.product(
      ...
      alteration=[
          ...
          # 'missing_pytree_data_dir_ocdbt.process_0',  # Not failing.
          'missing_pytree_data_dir_d',
      ],
  )
  def test_pytree_metadata_critical_corruptions(self, version, alteration):
    path = self.base_dir.joinpath(
        f'{version}_checkpoints',
        'composite_checkpoint',

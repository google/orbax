...
_VALID_STATE_PYTREES = (
    [1, 2.0, True],
    (b'bytes', 'string', None),
)

_INVALID_STATE_PYTREES = (
    test_tree_utils.MyClass(a=None, b=np.ones((2, 2))),
    test_tree_utils.MyDataClass(
        my_jax_array=None, my_np_array=np.ones((3, 3))
    ),
...
class ValidationTest(parameterized.TestCase):
  ...
  def test_validate_save_checkpointables_valid(self):
    validation.validate_save_checkpointables({'state': {'a': 1}})
    validation.validate_save_checkpointables({'stateful': {'a': 1}})
  ...
  def test_validate_state_basic_combinations(self, state):
  ...
  @parameterized.parameters(list(zip(_INVALID_STATE_PYTREES)))
  def test_validate_state_invalid_specific_containers(self, state):
    with self.assertRaises(ValueError):
      validation.validate_state(state)

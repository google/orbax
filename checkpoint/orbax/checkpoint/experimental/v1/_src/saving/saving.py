...
def _make_v0_save_args(
    ...
    array_storage_options_creator: (
        options_lib.ArrayStorageOptionsCreator | None
    ),
) -> tree_types.PyTree:
  """Creates v0-compatible `SaveArgs` from v1 storage options."""
  if array_storage_options_creator is None:
    save_args = None
  else:
    array_storage_options_pytree = jax.tree.map_with_path(
        array_storage_options_creator, pytree
    )
...
def save_pytree(
  ...
  save_args = _make_v0_save_args(
      pytree, context.pytree_options.array_storage_options_creator
  )

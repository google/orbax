...
def load_pytree(
  ...
  Returns:
    The restored PyTree.
  """
  def validate_checkpoint(path: path_types.PathLike, name: str | None):
    if name is None:
      # Checkpoint-level metadata is not used for loading in this case, and is
      # a non-standard place. Skip validation on this piece.
      format_utils.validate_checkpoint_directory(path)
      format_utils.validate_pytree_checkpoint(path, checkpointable_name=name)
    else:
      format_utils.validate_checkpoint_directory(path)
      format_utils.validate_pytree_checkpoint(path, checkpointable_name=name)
      format_utils.validate_checkpoint_metadata(path)

  validate_checkpoint_fn = functools.partial(
      validate_checkpoint, directory, checkpointable_name
  )
  ...
  return _load_checkpointables_impl(
      ...
      validate_checkpoint_fn=validate_checkpoint_fn,
  )[checkpointable_name]
...
def load_checkpointables(
  ...
  return _load_checkpointables_impl(
      directory,
      abstract_checkpointables,
      validate_checkpoint_fn=validate_checkpoint,
  )
...
def _load_checkpointables_impl(
    ...
    *,
    validate_checkpoint_fn: Callable[[], None],
) -> dict[str, Any]:
  ...
  Args:
    ...
    abstract_checkpointables: A dictionary of abstract checkpointables.
      Dictionary keys represent the names of the checkpointables, while the
      values are the abstract checkpointable objects themselves.
    validate_checkpoint_fn: A function that validates the checkpoint directory.

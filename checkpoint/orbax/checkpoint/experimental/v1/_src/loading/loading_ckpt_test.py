...
class LoadingCkptTest(absltest.TestCase):
  ...
  def test_load_orbax_checkpoint(self):
    loaded_checkpointables = loading.load_checkpointables(
        self.orbax_path / '0', abstract_checkpointables=None
    )
    self.assertIsInstance(loaded_checkpointables, dict)
    np.testing.assert_array_equal(
  ...
  def test_load_bad_path_orbax_ckpt(self):
    # User provides a specific Orbax checkpoint, not directory.
  ...
  def test_load_null_path(self):
    # User provides a null path.

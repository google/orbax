...
async def read(directory: Path) -> dict[str, Any]:
  """read.

  Helper method to read a safetensors checkpoint.
  ...
  """
  tensors = {}
  # TODO(zachmeyers): Use abstract_checkpointables for sharding logic.
  async with aiofiles.open(directory, mode="rb") as f:
    ...
    for name, info in header.items():
      try:
        dtype = dtype_dict[info["dtype"]]
      except KeyError as e:
        print(f"Can't parse this dtype {e}")
        continue
      shape = info["shape"]
...
class SafetensorsLayout(CheckpointLayout):
  ...
  async def load(
    ...
    del abstract_checkpointables  # Unused

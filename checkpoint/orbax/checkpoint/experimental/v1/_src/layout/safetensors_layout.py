...
async def _load_safetensors_on_device_gcs(
  ...
  # 3. Execute the highly parallel download
  # Using 64 processes handles the massive 17GB file effortlessly
  print("Starting parallel download to RAM disk...")
  print(
      f"----- [{path.name}] Single-shot network read from GCS starting:"
      f" {span_size / (1024 * 1024):.2f} MB in"
  )
  transfer_manager.download_chunks_concurrently(
      blob,
      ram_disk_path,
      chunk_size=256 * 1024 * 1024,  # 256 MB chunks per connection
      max_workers=64,  # Half of your 128 vCPUs
      worker_type="process",  # Crucial: Uses processes to bypass the GIL
  )
  ...
  print("Loading data into Python memory...")
  ...
  print(
      "Total time (Download + Memory Load):"
      f" {time.time() - start_read_time:.2f} seconds."
  )
...
async def _load_safetensors(
  ...
  if abstract_pytree is None:
    ...
    print(f"Unique files: {len(unique_files)}, files: {unique_files}")
  ...
  async def create_loading_plan(
    ...
    for _, header in all_headers:
      ...
      if current_batch_size + current_file_size > max_bytes:
        print(f"Batch size: {current_batch_size / (1024 * 1024):.2f} MB")

...
class OrbaxV0Layout(CheckpointLayout):
  ...
  async def _validate(self) -> None:
    ...
    """
    await self._orbax_layout.validate()
  ...
  async def _validate_pytree(self, checkpointable_name: str | None):
    ...
    """
    await self._orbax_layout.validate_pytree(checkpointable_name)
  ...
  async def load_pytree(
    ...
    return self._orbax_layout.load_pytree(checkpointable_name, abstract_pytree)

  async def load_checkpointables(
      self,
      abstract_checkpointables: dict[str, Any] | None = None,
  ) -> Awaitable[dict[str, Any]]:
    return self._orbax_layout.load_checkpointables(abstract_checkpointables)

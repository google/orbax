...
def _is_v0_checkpoint(layout: CheckpointLayout, path: path_types.Path) -> bool:
  # Using synchronous exists check here as this is a heuristic.
  # Ideally we should make this async, but for now we follow existing sync style
  # if path allows, or assume we can check existence synchronously if local.
  # However, path might be remote. `path.exists()` on epath might do RPC.
  # But `layout.has_indicator_file` was synchronous property before!
  # So it must have been doing sync IO or relying on cached state?
  # `OrbaxLayout.has_indicator_file` was `(self._path / ...).exists()`.
  # So it was sync.

...
class OrbaxLayout(CheckpointLayout):
  ...
  async def metadata(
    ...
    abstract_checkpointables = {
        name: None for name in existing_checkpointable_names
    }
    handlers_for_load = await get_handlers_for_load(
        path,
  ...
  async def load_pytree(
    ...
    if handler_for_load is None:
      raise ValueError(
          "Failed to resolve handler for checkpointable"
  ...
  async def load_checkpointables(
    ...
    Raises:
      ...
      the checkpoint.
      ValueError: If an empty checkpoint is attempted to be loaded.
    """
    existing_checkpointable_names = await _existing_checkpointable_names(path)
    if not existing_checkpointable_names:
      raise ValueError(
          "Checkpoint is empty, no checkpointables to load."
      )
    ...
    for name, abstract_checkpointable in abstract_checkpointables.items():
      if name not in checkpoint_layout.RESERVED_CHECKPOINTABLE_KEYS:
        if name in existing_checkpointable_names:
          checkpointable_names_to_load[name] = abstract_checkpointable
      else:
        logging.warning(
            "Checkpointable name %s not found in checkpoint. Proceeding to"
            " load remaining checkpointables.",
            name,
        )

    if not checkpointable_names_to_load:
      raise ValueError(
          "Failed to load requested checkpointables. Available subdirectories"
          " that may be interpreted as checkpointables:"
    ...
    ) in checkpointable_names_to_load.items():
      handler = handlers_for_load[checkpointable_name]
      if handler is None:
        load_ops.append(None)
      else:
        load_ops.append(
            handler.load(
                path / checkpointable_name,
    ...
    load_awaitables = await asyncio.gather(*load_ops, return_exceptions=True)
    ...
    async def _run_background() -> dict[str, Any]:
      ...
      for name, a in zip(checkpointable_names_to_load.keys(), load_awaitables):
        if a is None:
          loaded_checkpointables[name] = None
        elif isinstance(a, BaseException):
          logging.warning(
              "Failed to load checkpointable '%s' due to initial load setup"
              " failure with incompatible handler: %s. Defaulting return value"
              " to None. \nError details: %s",
              name,
              handlers_for_load[name],
              a,
          )
          loaded_checkpointables[name] = None
        else:
          try:
            ...
            logging.warning(
                "Failed to load checkpointable during load operation '%s' due"
                " to incompatible handler: %s for abstract checkpointable: %s."
                " Defaulting return value to None. \nError details: %s",
                name,
                handlers_for_load[name],
                abstract_checkpointable,
                e,
  ...
  async def save(
    ...
    handlers_for_save = get_handlers_for_save(
...
async def get_handlers_for_load(
    ...
    checkpoint_metadata: InternalCheckpointMetadata | None = None,
  ...
  1. Check checkpointable_name explicit registration first.
  2. Check for handler_typestr in checkpoint metadata item_handlers using
  ...
  Args:
    ...
    abstract_checkpointables: The abstract checkpointables to load.
    checkpoint_metadata: Optional InternalCheckpointMetadata if already loaded.

  Returns:
    A mapping from checkpointable name to handler.
  """
  # If abstract_checkpointables is not provided, we will infer the
  # checkpointable names from the metadata or from the subdirectories.
  if not abstract_checkpointables:
    if checkpoint_metadata is None:
      checkpoint_metadata = await read_checkpoint_metadata(directory)
    _validate_checkpoint_metadata(checkpoint_metadata)
    abstract_checkpointables = await _get_valid_checkpointable_names_to_load(
        directory, checkpoint_metadata
    )
  ...
  for (
    ...
    # 2. Find handler_typestr in checkpoint metadata item_handlers using
    # checkpointable_name as key.
    if checkpoint_metadata is None:
      checkpoint_metadata = await read_checkpoint_metadata(directory)
      _validate_checkpoint_metadata(checkpoint_metadata)
...
async def _get_valid_checkpointable_names_to_load(
    directory: path_types.Path,
    checkpoint_metadata: InternalCheckpointMetadata,
) -> dict[str, Any]:
  """Returns all checkpointables that we can load from this current checkpoint.

  In the case that abstract_checkpointables is not provided, we will return a
  mapping of all the checkpointables that we can load from this current
  checkpoint.
  1. Use item_handlers keys from checkpoint metadata as the checkpointable
  names to load, if checkpoint contains subdirectories of the same name.
  2. If metadata does not exist or does not contain item_handlers keys, get
  checkpointable_names from checkpoint subdirectories.

  Args:
    directory: The path to the checkpoint.
    checkpoint_metadata: The InternalCheckpointMetadata to load from.

  Returns:
    A mapping from checkpointable name to abstract checkpointable.
  """
  if isinstance(checkpoint_metadata.item_handlers, dict):
    return {name: None for name in checkpoint_metadata.item_handlers.keys()}
  else:
    logging.warning(
        "No valid item handler checkpoint metadata found in '%s'. Inferring"
        " checkpointable_names from subdirectories to resolve handler type"
        " strings.",
        directory,
    )
  existing_checkpointable_names = await _existing_checkpointable_names(
      directory
  )
  return {
      name: None
      for name in existing_checkpointable_names
  }

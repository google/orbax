...
import asyncio  # why is asyncio not being used here?
...
class OrbaxLayout(CheckpointLayout):
  ...
  def __init__(
      self, handler_registry: registration.CheckpointableHandlerRegistry
  ):
    ...
    self.composite_handler_loading = CompositeHandler(self._handler_registry)

  async def validate(self, path: Path):
    try:
      if not await asyncio.to_thread((path / '_CHECKPOINT_METADATA').exists):
        # TODO(zachmeyers): update to orbax.checkpoint once the CL is submitted.
        return False
      await asyncio.to_thread(format_utils.validate_checkpoint_directory, path)
      await asyncio.to_thread(format_utils.validate_checkpoint_metadata, path)
  ...
  async def load(
    ...
    load_awaitable = await self.composite_handler_loading.load(

...
class OrbaxLayout(CheckpointLayout):
  ...
  def has_indicator_file(self, path: Path) -> bool:
    return (path / ORBAX_CHECKPOINT_INDICATOR_FILE).exists()
  ...
  async def _validate_pytree(self, path: Path, checkpointable_name: str | None):
    ...
    if checkpointable_name is None:
      ...
      if not await async_path.exists(pytree_dir):
        if await _has_pytree_metadata_file(path):
          pytree_dir = path
        else:
          subdirs = [
              d.name
              for d in await _subpaths(path)

...
class OrbaxLayout(CheckpointLayout):
  ...
  async def _validate_pytree(self, checkpointable_name: str | None):
    ...
    if (
        checkpoint_version(self.path) == CheckpointVersion.V1
    ...
    if not await async_path.exists(
      ...
      for subdir in await _subpaths(self.path):
      ...
      if await has_pytree_metadata_file(self.path):
  ...
  async def load_pytree(
    ...
    typestr = self._get_typestr(self.path, checkpointable_name)
    name_for_registration = checkpointable_name or self.path.name
    ...
    # if it has _METADATA
    elif checkpointable_name is None and await has_pytree_metadata_file(
        self.path
    ):
      ...
      raise ValueError(
          ...
          f" name: {checkpointable_name} in path: {self.path}."
      )

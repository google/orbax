...
class OrbaxV0LayoutTest(
  ...
  async def test_is_v0_orbax_checkpoint(self):
    ...
    self.assertTrue(
        await loop.run_in_executor(
            None, orbax_v0_layout.is_orbax_v0_checkpoint, self.orbax_path / '0'
        )
    )
    self.assertFalse(
        await loop.run_in_executor(
            None, orbax_v0_layout.is_orbax_v0_checkpoint, self.safetensors_path
        )
    )
    self.assertTrue(
        await loop.run_in_executor(
            None, orbax_v0_layout.is_orbax_v0_checkpoint, cm_dir / '0'
        )
    )

...
class OrbaxV0LayoutTest(
  ...
  @parameterized.named_parameters(
      ('with_metadata', False, 'state'),
      ('no_metadata', True, 'state'),
      ('with_metadata_none_name', False, None),
      ('no_metadata_none_name', True, None),
  )
  async def test_metadata(self, remove_metadata, checkpointable_name):
    if remove_metadata:
      await async_path.unlink(self.orbax_path / '0' / '_CHECKPOINT_METADATA')

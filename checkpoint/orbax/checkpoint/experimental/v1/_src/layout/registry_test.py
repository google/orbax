...
class DetectLayoutTest(
  ...
  async def test_auto_save_path_resolves_to_orbax(self):
    cls = await registry.get_layout_class(CheckpointLayoutEnum.AUTO, path=None)
    self.assertEqual(cls, orbax_layout.OrbaxLayout)

  async def test_auto_with_path(self):
    cls_v1 = await registry.get_layout_class(
        CheckpointLayoutEnum.AUTO, path=self.v1_directory
    )
    self.assertEqual(cls_v1, orbax_layout.OrbaxLayout)

    cls_v0 = await registry.get_layout_class(
        CheckpointLayoutEnum.AUTO, path=self.v0_directory
    )
    self.assertEqual(cls_v0, orbax_v0_layout.OrbaxV0Layout)

    cls_st = await registry.get_layout_class(
        CheckpointLayoutEnum.AUTO, path=self.safetensors_dir
    )
    self.assertEqual(cls_st, safetensors_layout.SafetensorsLayout)

  async def test_resolver_with_auto(self):
    resolver = await registry.CheckpointLayoutResolver.resolve(
        self.v1_directory,
        CheckpointLayoutEnum.AUTO,
  ...
  async def test_nonexistent_safetensors_raises_invalid_layout(self):
    missing_st = self.root_directory / 'nonexistent.safetensors'
    with self.assertRaises(registry.InvalidLayoutError):
      await registry.detect_layout(missing_st)

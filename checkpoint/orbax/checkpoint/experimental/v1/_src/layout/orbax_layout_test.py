...
class OrbaxLayoutTest(unittest.IsolatedAsyncioTestCase, parameterized.TestCase):
  ...
  async def test_is_v1_orbax_checkpoint(self):
    ...
    self.assertTrue(
        await loop.run_in_executor(
            None, orbax_layout.is_orbax_v1_checkpoint, self.orbax_path / '0'
        )
    )
    self.assertFalse(
        await loop.run_in_executor(
            None, orbax_layout.is_orbax_v1_checkpoint, self.safetensors_path
        )
    )
    # This is not a valid V1 checkpoint as it does not have the indicator file
    self.assertFalse(
        await loop.run_in_executor(
            None, orbax_layout.is_orbax_v1_checkpoint, cm_dir / '0'
        )
    )

...
Foo = handler_test_utils.Foo
AbstractFoo = handler_test_utils.AbstractFoo
...
class LeafHandlerTest(parameterized.TestCase):
  ...
  def test_save_load(self, handler_cls):
    ...
    self.assertFalse(handler.is_handleable(Foo(1, 'hi')))
    self.assertFalse(
        handler.is_abstract_handleable(AbstractFoo())
    )
  ...
  def test_unregistered_type(self):
    ...
    with self.assertRaises(registry.UnregisteredTypeError):
      handler.save(self.directory, Foo(1, 'hi'))

    with self.assertRaises(registry.UnregisteredTypeError):
      handler.load(self.directory, AbstractFoo())

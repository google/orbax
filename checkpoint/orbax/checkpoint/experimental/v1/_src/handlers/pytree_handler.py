...
async def _async_futures(
  ...
  def _wait_with_timeout(f: future.Future):
    if timeout_secs is None or start_time is None:
      return f.result()
    time_remaining_secs = max(0.0, timeout_secs - (time.time() - start_time))
    if time_remaining_secs <= 0:
      raise TimeoutError('Overall save timeout exceeded.')
    return f.result(timeout=time_remaining_secs)
...
class PyTreeHandler(CheckpointableHandler[PyTree, PyTree]):
  ...
  **Note: Users are encouraged NEVER to instantiate or use this handler
  ...
  async def _background_save(
    ...
    time_remaining_secs = max(0.0, timeout_secs - (time.time() - start_time))
    if time_remaining_secs <= 0:
      raise TimeoutError('Timed out while waiting for commit to complete.')
    await multihost.sync_global_processes(
        barrier_name,
        operation_id=operation_id,
        processes=active_processes,
        timeout=int(time_remaining_secs),
    )
    # Finalize.
    ...
    time_remaining_secs = max(0.0, timeout_secs - (time.time() - start_time))
    if time_remaining_secs <= 0:
      raise TimeoutError('Timed out while waiting for finalize to complete.')
    await multihost.sync_global_processes(
        barrier_name,
        operation_id=operation_id,
        processes=active_processes,

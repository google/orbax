...
If a registered handler is scoped to a specific name 
(e.g. `registry.add(BazHandler, 'baz')`), then this handler will always be
...
import logging
from typing import Any, Callable, Protocol, Sequence, Type
...
class _DefaultCheckpointableHandlerRegistry(CheckpointableHandlerRegistry):
  ...
  def has(
    ...
    Returns:
      ...
      otherwise.
    """
    logging.info(self.get_all_entries())
    try:
...
def register_handler(handler_type: Type[CheckpointableHandler]):
  _GLOBAL_REGISTRY.add(handler_type)

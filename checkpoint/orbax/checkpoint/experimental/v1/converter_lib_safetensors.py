...
"""Converter for Safetensors checkpoint format to native Orbax format."""
...
async def create_loading_plan(
  ...
  max_bytes = _MAX_BATCH_SIZE_MB.value * 1024 * 1024
  total_tensors = 0
...
async def get_tensor_to_path_indexing(paths: Sequence[Path]) -> dict[str, Path]:
  ...
  tensor_to_path = {}
  if not await asyncio.to_thread(path_.exists):
    for path in paths:
  ...
  raw_data = await asyncio.to_thread(path_.read_bytes)
...
async def _execute_batch_async(
  ...
  for path, abstract_pytree in file_abstract_trees.items():
    ...
    with ocp.Context(
      ...
      loaded = ocp.load_pytree_async(
          path=Path(path),
          abstract_pytree=abstract_pytree,
          checkpointable_name=None,
      )
      current_loaded_tensors = loaded.result()
...
async def run_cpu_batching(input_dir: str, output_dir: str):
  ...
  with ocp.Context(checkpoint_layout=ocp.options.CheckpointLayout.SAFETENSORS):
    metadata_ckpt = ocp.pytree_metadata(input_path)
  ...
  load_start_time = time.time()
  loaded_tensors = await _load_safetensors(input_path)
  ...
  _log_info(
      ...
      (total_load_size / (time.time() - load_start_time)),
  )
  if _SAVING_ENABLED.value:
    ...
    with ocp.Context(checkpoint_layout=ocp.options.CheckpointLayout.ORBAX):
    ...
    _log_info(
        '\033[1mSaved Native Orbax Checkpoint Time: %.2f seconds\033[0m',
        total_save_time,
    )

...
def assert_every_n_is_x_apart(testclass, values, n, x):
  ...
  # asserts that each element in a group is at least x greater than the
  # corresponding element of the previous group (which released the bytes
  # allowing it to proceed), allowing a small tolerance for timer slack.
  values = sorted(values)
  assert len(values) % n == 0
  for i in range(n, len(values)):
    testclass.assertGreaterEqual(values[i], values[i - n] + x * 0.95)

...
def _get_param_names(params: PyTree) -> PyTree:
  ...
  for k, v in flattened.items():
    if v is traverse_util.empty_node:
      flat_names_dict[k] = {}
    else:
      flat_names_dict[k] = '.'.join(k)
  names = serialization.from_state_dict(
      params, traverse_util.unflatten_dict(flat_names_dict)
  )
  ...
  if jax.tree_util.tree_structure(params) != jax.tree_util.tree_structure(
    ...
    logging.warning(
        (
            ...
            ' ``flax.serialization.register_serialization_state``. PyTreeDef of'
            ' params=%s'
        ),

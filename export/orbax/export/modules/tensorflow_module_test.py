...
class TensorFlowModuleTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_variable_names(self, top_level_dict_cls=dict):
    ...
    variable_names_to_vals = {
        ...
        for v in TensorFlowModule(
            params=params, apply_fn=lambda params, x: x
        ).variables
    }
    self.assertEqual(
        {
  ...
  def test_trainable(self):
    ...
    jm = TensorFlowModule(
        params=params, apply_fn=lambda params, x: x, trainable=trainable
    )
    ...
    jm = TensorFlowModule(params=params, apply_fn=lambda params, x: x)
    self.assertEmpty(jm.trainable_variables)
    self.assertFalse(jm.with_gradient)

    jm = TensorFlowModule(
        params=params, apply_fn=lambda params, x: x, trainable=True
    )
    self.assertLen(jm.trainable_variables, 2)
    self.assertTrue(jm.with_gradient)

    jm = TensorFlowModule(
        params=params, apply_fn=lambda params, x: x, trainable=False
    )
    self.assertEmpty(jm.trainable_variables)
    self.assertFalse(jm.with_gradient)
  ...
  def test_jax_array(self):
    ...
    variables = TensorFlowModule(
        params={'arr': arr}, apply_fn=lambda params, x: x
    ).variables
  ...
  def test_call_tf_module_methods(self, jit_compile):
    ...
    tf_module = TensorFlowModule(
        params=params, apply_fn=linear, jit_compile=jit_compile
    )
  ...
  def test_variable_update(self):
    ...
    tf_module = TensorFlowModule(
        params=params, apply_fn=linear, input_polymorphic_shape='b, ...'
    )
  ...
  def test_variable_update_error(self):
    ...
    tf_module = TensorFlowModule(
        params=params, apply_fn=lambda params, x: params['w'] @ x
    )
  ...
  def test_save_load_as_jax_exported_map(self):
    ...
    chex.assert_equal(
        set(
            restored_jax_exported_map[DEFAULT_METHOD_KEY].platforms
        ),

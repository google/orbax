...
class ExportManager:
  ...
  def save(
    ...
    """
    # Warn the user if they are attempting to sign an Orbax model.
    # TODO(b/398254444): Remove this warning once tree verity is supported.
    if (
        self._version == constants.ExportModelType.ORBAX_MODEL
        and tree_verity_options
        and tree_verity_options.attestation_signing
        != tree_verity.AttestationSigning.SIGNING_DISABLED
    ):
      logging.warning(
          'go/tree-verity was enabled but is not currently supported for Orbax'
          ' models and will be ignored. See b/398254444 for status.'
      )

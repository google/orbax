...
class ExportManager:
  ...
  def __init__(
    ...
    self._module = module
    self._export_fncs = tensorflow_export.TensorFlowExport(
        module, serving_configs
    )
    if self._version == constants.ExportModelType.ORBAX_MODEL:
      self._export_fncs = obm_export.ObmExport(
          module, serving_configs
      )
  ...
  def serving_signatures(self) -> Mapping[str, Callable[..., Any]]:
    ...
    return self._export_fncs.serving_signatures
  ...
  def load(self, model_path: str, **kwargs: Any):
    loaded = self._export_fncs.load(model_path, **kwargs)
    return loaded

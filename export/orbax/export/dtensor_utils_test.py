...
class DtensorUtilsTest(parameterized.TestCase):
  ...
  def test_jax_array_to_dtensor(
    ...
    dt_arr = dtensor_utils.jax_array_to_dtensor(
        jax_arr, pspec, self._mesh, dmesh
    )
  ...
  def test_sharding_across_multi_mesh_axes(
    ...
    dt_arr = dtensor_utils.jax_array_to_dtensor(
        jax_arr, pspec, mesh, dmesh, allow_multi_axis_sharding_conslidation=True
    )
  ...
  def test_jax_dtensor_mesh_mismatch(self):
    ...
    with self.assertRaisesRegex(
      ...
      dtensor_utils.jax_array_to_dtensor(jax_arr, pspec, self._mesh, dmesh)

  def test_get_current_dtensor_mesh(self):
    self.assertIsNone(dtensor_utils.get_current_mesh())
    with dtensor_utils.maybe_enable_dtensor_export_on(self._mesh):
      mesh = dtensor_utils.get_current_mesh()
      self.assertIsInstance(mesh, dtensor_utils.Mesh)
      self.assertIsInstance(mesh.jax_mesh, jax.sharding.Mesh)
      self.assertIsInstance(mesh.dtensor_mesh, dtensor.Mesh)
    self.assertIsNone(dtensor_utils.get_current_mesh())

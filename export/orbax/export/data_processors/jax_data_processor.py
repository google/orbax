...
def _jax_spec_from(spec: Any) -> jax.ShapeDtypeStruct:
  ...
  The None in the shape is converted to a string "None", which will be later
  converted to a symbolic dimension.

  Args:
    spec: The spec to convert.

  Returns:
    The converted jax.ShapeDtypeStruct.
  """
  if isinstance(spec, shlo_function.ShloTensorSpec):
    if spec.dtype == shlo_function.ShloDType.bf16:
      return jax.ShapeDtypeStruct(
          _convert_shape(spec.shape), jax.numpy.bfloat16
      )
    return jax.ShapeDtypeStruct(

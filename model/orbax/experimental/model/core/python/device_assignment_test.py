...
class DeviceAssignmentTest(absltest.TestCase):

  def test_device_assignment_dataclass(self):
    assignment = device_assignment.DeviceAssignment(
        id=0, coords=(1, 2), core_on_chip=3
    )
    self.assertEqual(assignment.id, 0)
    self.assertEqual(assignment.coords, (1, 2))
    self.assertEqual(assignment.core_on_chip, 3)

  def test_device_assignment_with_defaults(self):
    assignment = device_assignment.DeviceAssignment(id=0)
    self.assertEqual(assignment.id, 0)
    self.assertIsNone(assignment.coords)
    self.assertIsNone(assignment.core_on_chip)

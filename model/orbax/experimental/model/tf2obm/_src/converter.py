...
def to_keyword_only_fn(
  ...
  Returns:
    The wrapped function. Returned as a polymorphic function to ensure the
    defaults from the original polymorphic function are preserved. The
    wrapped function always has an input signature, so it can be concretized
    at any point by calling `get_concrete_function()` without arguments.
  """
  if isinstance(f, tf.types.experimental.PolymorphicFunction):
    if f.input_signature is None:
      raise ValueError(
          'OBM can serialize a TF PolymorphicFunction only if it provides the'
          ' input signature'
      )
    concrete_fn = f.get_concrete_function()
  else:
    concrete_fn = f

  input_names, input_leaves, input_def = _flat_input_signature(concrete_fn)
  ...
  new_input_signature_with_input_names = {
      key: tf.TensorSpec(shape=spec.shape, dtype=spec.dtype, name=name)
      for (key, spec), name in zip(new_input_signature.items(), input_names)
  }

  @tf.function(
      autograph=False,
      input_signature=[new_input_signature_with_input_names],
  )
...
def save_tf_functions(
  ...
  # convert_function).
  wrapped_fns = {name: to_keyword_only_fn(fn) for name, fn in fns.items()}

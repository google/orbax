  ...
  def test_export_mnist(self, batch_size):
    with flagsaver.flagsaver(batch_size=batch_size):
      with self.assertLogs(level="INFO") as logs:
        export_mnist()
      # Exactly one log entry says that the model from the output dir is loaded.
      self.assertEqual(
          sum(f"Loaded the model from {_FLAGS.output_dir}" in log_entry
              for log_entry in logs.output), 1)

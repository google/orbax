...
class CheckpointerTestBase:
  ...
  class Test(parameterized.TestCase):
    ...
    def wait_if_async(self, ckptr):
      if isinstance(ckptr, AsyncCheckpointer):
        ckptr.wait_until_finished()

    def test_save_restore(self):
      """Basic save and restore test."""
      ckptr = self.checkpointer(PyTreeCheckpointHandler())
      ckptr.save(self.directory, self.pytree)
    ...
    def test_restore_missing_path(self):
      ...
      with self.assertRaises(FileNotFoundError):
        ckptr.restore('path/to/missing')

    def test_save_structure(self):
      ckptr = self.checkpointer(PyTreeCheckpointHandler())
      ckptr.save(self.directory, self.pytree)
      self.wait_if_async(ckptr)
      structure = ckptr.structure(self.directory)
      handler = PyTreeCheckpointHandler()
      expected = ckptr.structure(handler.structure(self.directory))
    ...
    def test_no_overwrite_existing(self):
      ...
      with self.assertRaises(ValueError):
        ckptr.save(self.directory, self.doubled_pytree)
      self.wait_if_async(ckptr)
      restored = ckptr.restore(
          self.directory, restore_args=self.pytree_restore_args)
    ...
    def test_flax_train_state(self):
      ...
      ckptr = self.checkpointer(PyTreeCheckpointHandler())
      ckptr.save(self.directory, state)
      self.wait_if_async(ckptr)
    ...
    def test_other_checkpointers(self):
      pass
